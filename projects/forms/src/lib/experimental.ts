...
interface TypedFormArray2<T extends object|string|number|boolean, K extends Array<T> = T[]> extends FormArray {
  valueChanges: Observable<K>;
  statusChanges: Observable<'VALID' | 'INVALID' | 'PENDING' | 'DISABLED'>;
  patchValue: (v: K, options?: FormEventOptions) => void;
  setValue: (v: K, options?: FormEventOptions) => void;
...
// let t4: TypedFormArray2<string>;
// t4.valueChanges.subscribe(v => )
/**
...
function typedFormArray2<T extends object|string|number|boolean>(
  controls: Array<TypedFormGroup<T> | TypedFormControl<T> | TypedFormArray2<T>>,
  validatorOrOptions?: ValidatorFn | ValidatorFn[] | AbstractControlOptions | null,
  asyncValidators?: AsyncValidatorFn | AsyncValidatorFn[] | null
): TypedFormArray2<T> {
  return new FormArray(controls, validatorOrOptions, asyncValidators) as any;
...
const c = typedFormArray2<PersonContact>([typedFormControl({name: '', email: ''})]);
c.controls.
